...
def resolve_da3_backbone_defaults(model_path_or_name: str, variant: str = "auto") -> dict[str, int | tuple[int, ...]]:
    ...
    if normalized_variant not in DA3_BACKBONE_DEFAULTS:
        raise ValueError(f"Unsupported DA3 variant {variant!r}. Expected one of: auto, large, giant.")
    ...
    try:
        ...
        if code_root is None:
            raise ImportError(
                ...
                "`framework.future3d.da3_code_root` to a standalone DA3 repository root or its `src` directory."
            ) from first_exc
        ...
        if not candidate_src.exists():
            raise FileNotFoundError(
                f"framework.future3d.da3_code_root={code_root!r} does not exist or does not contain a `src` directory"
            )
        ...
        try:
            ...
            raise ImportError(
                "Failed to import DepthAnything3 from the configured `framework.future3d.da3_code_root`. "
...
class DA3BackboneTeacher(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> list[torch.Tensor]:
        ...
        x = x.view(batch_size, num_views, channels, self.process_res, self.process_res)

        self._log_attention_backend_once(x)
        features_tuple, _ = self.backbone(x)
        layer_outputs = {
            layer_idx: layer_out for layer_idx, layer_out in zip(self.out_layers, features_tuple, strict=False)
        }
        teacher_features = []
        for layer_idx in self.teacher_layers:
            layer_out = layer_outputs[layer_idx]
            patch_tokens = layer_out[0]
            teacher_features.append(patch_tokens.reshape(batch_size, -1, patch_tokens.shape[-1]))
